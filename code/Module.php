...
class CheckfrontModule extends Object implements CheckfrontAPIInterface {
     ...
     * @return null|string
     */
    public static function encode_link_segment($accessKey, $itemID, $startDate, $endDate) {
        try {
            $values = array(
                self::TokenItemIDIndex => $itemID,
                self::TokenStartDateIndex => $startDate,
                self::TokenEndDateIndex => $endDate,
                self::TokenAccessKeyIndex => $accessKey
            );
            $value = implode(
            ...
            return Crypto::encrypt($value, $accessKey);
        } catch (Exception $e) {
            return null;
        }
    }
     ...
     * parameters passed to self.encode_link_segment call.
     *
     * @param $link
     * @param null $index - optionally only return a single value from decoded array
     *
     * @return null|array - [packageID, startDate, endDate, accessKey]
     */
    public static function decode_link_segment($link, $index = null) {
        try {
            $values = explode(
                self::LinkPartDelimeter,
     ...
     * base64_encoded/decoded if presented to the client for copy/paste etc.
     *
     * @return string
     */
    public static function make_access_key() {
        $crypto = new Crypto();
